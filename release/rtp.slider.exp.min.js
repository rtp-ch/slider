...
(this.finger={},this.fingers=0,this.ordered.length=0);m=a.inArray(this,e[c]);-1!=m&&e[c].splice(m,1);0==e[c].length&&(delete g[c],delete e[c])}};b.getCenter=function(){var b={x:0,y:0},c;for(c in this.finger){var a=this.finger[c];b.x+=a.x;b.y+=a.y}this.fingers&&(b.x/=this.fingers,b.y/=this.fingers);return b};b.getRotation=function(){if(2>this.fingers)return 0;for(var b=0,c=0,a=0,e=0,f=this.ordered.length;e<f;e++)for(var h=this.finger[this.ordered[e]],g=e+1;g<f;g++){a++;var k=this.finger[this.ordered[g]],
...
c){var m=a(c).data("gesture");if("undefined"==typeof m)m=new OCBNET.Gestures(c,b),a(c).data("gesture",m);else{if(0<m.fingers)throw"fatal: live config change";a.extend(m.config,b,!0)}});return this}})(jQuery);
...
(function(b){b.init.push(function(b){var d=[],f=this.config["native"]||{};f.panY&&d.push("pan-y");f.panX&&d.push("pan-x");0==d.length&&d.push("none");f=a.proxy(g,this);a(b).bind(e.down,f).css(e.action,d.join(" "))})})(OCBNET.Gestures.prototype);var b=[e.up,e.cancel].join(" ");a(document).bind(b,h);a(document).bind(e.move,f)}})(jQuery);
